...
var webssoLoginFlow = function webssoLoginFlow() {
   ...
   if (windows_auth == "true") {
      if (!isMac && !isLinux) {
         ...
         checkboxDisableChange(sspiCheckbox);
      }
...
function onGetADUserName(result, err) {
  ...
  // 1st leg of Negotiate SSPI with initialize phase
  doLog("onGetADUserName is successful for user - " + this.userName + ". now start the kerberos authentication initialize phase");
...
var federationLoginFlow = function federationLoginFlow() {
   var prefilledUsername = getQueryParam('prefilledUsername');
   var discovery = getQueryParam('discovery');
   var isConsentChecked = getQueryParam('isConsentChecked');

   // check if username is pre-populated and
   // discovery (indicate redirection from discovery page)
   // else continue with the existing flow.
   if (!!prefilledUsername && discovery && discovery.trim() === 'true') {
      isFederationLogin = true;
      //enable the consent checkbox if it's redirected from discovery page and consent is checked already
      if (isConsentChecked && isConsentChecked.trim() === 'true') {
         document.getElementById('logonBannerCheckbox').checked = true;
      }
      //display pre-populated username , hide all checkbox options for authentication and logon consent
      document.getElementById("username").value = prefilledUsername;
      document.getElementById("username").readOnly = 'true';
      document.getElementById("password").focus();
      //if external IDP is configured, allow only system domain login
      tlsclient_auth = windows_auth = rsa_am_auth = "false";
   }
