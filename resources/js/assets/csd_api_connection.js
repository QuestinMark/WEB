...
// global
var VMW_CSD_DEFAULT_WSS_PORT = 8094;
...
function ApiConnection() {
   ...
   this.open = function(args) {
      ...
      // for FF, we start protocol server before websocket
      // so that all logged in users have read permission to FF publick key cert
   ...
   this.__tryOpen__ = function(method, port) {
      var me = this;
      try {
         var socket = new WebSocket(method + "://vmware-plugin:" + port + "/" +
   ...
   this.__callStartProtocolServer__ = function() {
      var pUrl = 'vmware-plugin://csd?sessionId=' + this._sessionId +
            '&appName=' + this.appName +
            '&version=' + this._csdVersion;
